...
- `screen2-*` … `screen6-*`, `credential-*` — the onboarding screens (Screen2–6, ScreenCredential). **Reachable since 2026-09-20**: `OnboardingFlow.jsx` is imported and mounted by `App.jsx` for any signed-in profile whose `onboarding_completed` is false (a brand new signup, including a Google SSO one). Before that fix the gate existed without the import, so the element was a free identifier and every such account got a blank white page. No flow in `flows.json` exercises them yet; sign-up today goes straight from `screen0-get-started` to the Auth form, and the flow only appears once a session exists.
