...
function CoordinatorIcon({ selected }) {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
                ...
                <span className="flex items-center gap-3.5">
                  <span
                    className={`flex size-[46px] shrink-0 items-center justify-center rounded-full ${
                      selected ? 'bg-[#DAF4F9]' : 'bg-[#F2F2F2]'
                    }`}
                  >
                    {isCoordinator ? (
                      <CoordinatorIcon selected={selected} />
                    ) : (
                      <NurseIcon selected={selected} />
                    )}
                  </span>
                  <span>
                    <span
                      className={`block text-[17px] font-semibold ${
                        selected ? 'text-[#003342]' : 'text-[#004458]'
                      }`}
                    >
                      {option.label}
                    </span>
